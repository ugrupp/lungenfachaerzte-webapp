...
- **Tailwind v4**: no `tailwind.config.ts` — CSS is in `src/styles.css` only. Use `@source` directives or `content` globs inside the CSS file if classes are not detected.  
...
skills:
  ...
  - task: "server/client boundary, isomorphic code, environment variables, VITE_ prefix"
  ...
  - task: "authentication, route guards, beforeLoad redirects"
